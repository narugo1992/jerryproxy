...
def unlink_windows_identity_guard_path(guard):
    ...
    handle = kernel.CreateFileW(
        ...
        (
            removal_module._WINDOWS_FILE_FLAG_BACKUP_SEMANTICS
            | removal_module._WINDOWS_FILE_FLAG_OPEN_REPARSE_POINT
        ),
        None,
    )
...
def test_windows_symlink_failure_uses_and_replaces_a_verified_copy(tmp_path, monkeypatch):
    ...
    with pytest.raises(BackendNotInstalledError, match="copy failed integrity verification"):
        manager.which("mihomo")
