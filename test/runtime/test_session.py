...
def test_runtime_authentication_is_explicit_and_published_for_local_clients(tmp_path):
    record = _record(nodes=1)
...
def test_runtime_bind_all_publishes_wildcard_listener_and_mihomo_lan_config(tmp_path):
    record = _record(nodes=1)
    ...
    access = runtime.access_path.read_text(encoding="ascii")
    config = runtime.config_path.read_text(encoding="utf-8")
    assert '"address":"0.0.0.0"' in access
    assert runtime.public_info()["listener"]["address"] == "0.0.0.0"
