...
URI_TEMPLATE = "ss://YWVzLTI1Ni1nY206cGFzc3dvcmRAMTkyLjAuMi4x:%d#node-%d\n"
...
class FakeProcess(object):
    def __init__(self, executable, config_path, session_root, log_path, backend_log_level):
        del executable, config_path, session_root, log_path, backend_log_level
...
def _session(tmp_path, record, probe, manager=None, policy=None):
...
def test_runtime_accepts_a_driver_without_changing_session_ownership(tmp_path):
    class FakeDriver(RuntimeDriver):
        ...
        def projection(self, provider_path, node, port, username, password):
            del provider_path, node, port, username, password
