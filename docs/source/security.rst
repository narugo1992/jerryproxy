...
* backend process output is drained as one merged stream, redacted before
  persistence, and labeled only with the backend name (for example
  ``[mihomo]``); JerryProxy-owned records have no owner prefix;
