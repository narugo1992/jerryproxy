...
Forms:
  ...
  jerryproxy server --subscription NAME --node NODE_ID --port 1080
...
`--protocol` selects the local listener: `mixed` accepts HTTP and SOCKS5,
`http` accepts HTTP proxy clients, and `socks5` accepts SOCKS5 clients. The
listener binds to `127.0.0.1` by default and is unauthenticated for a simple
local proxy entry point. `--bind-all` changes the bind address to `0.0.0.0`
and enables Mihomo LAN access; this exposes the unauthenticated listener to
every reachable interface unless `--auth` is also supplied. `--auth` enables
random credentials in the private access file. JerryProxy messages have no
owner prefix; backend output is merged into one stream and labeled only with
the backend name (`[mihomo]`, `[v2ray]`, and so on) in the terminal and private
timestamped runtime log. The stream never distinguishes the child's original
stdout and stderr.

Human startup output is emitted through the JerryProxy log stream as one
readable readiness summary, one copyable proxy URL, and a short multi-line
environment-variable guide. With `--auth`, the guide also explains the
generated credentials once; JSON output keeps credentials in the private
access file only. The private runtime log filename contains its UTC start time
to the second. The guide assigns the same URL to `HTTP_PROXY`, `HTTPS_PROXY`,
and `ALL_PROXY`; SOCKS5 uses a `socks5h` URL so DNS follows the proxy too. In
guided mode, the port input checks the default first and
pre-fills a safe alternate when the default is busy. Press Enter to accept the
displayed port, type `auto` for automatic allocation, or enter another port
number.
"""
...
@click.option(
    ...
    default=False,
    show_default=True,
    help="Require random local proxy credentials; default is an open loopback listener.",
)
@click.option(
    "--bind-all",
    is_flag=True,
...
@click.option(
    ...
    default="INFO",
    show_default=True,
    help="Capture and echo backend output at this level; OFF disables capture.",
)
...
def server_command(
    ...
    def log_sink(source, level, message, emphasize=False):
    ...
    def startup_log(message, emphasize=False):
        log_sink("jerryproxy", "INFO", message, emphasize=emphasize)
    ...
    try:
        ...
        if log_format == "jsonl":
            ...
            startup_log("Proxy URL: %s" % proxy_url, emphasize=True)
            guide = [
